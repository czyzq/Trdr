...
export const ChartTest: React.FC<ChartTestProps> = ({ symbol = "XAU" }) => {
  ...
  const testChartData = async () => {
    try {
      ...
      const response = await fetch(
        `/api/chart/${symbol}?resolution=60&count=10`,
      );
      ...
      const missingFields = requiredFields.filter(
        (field) => !(field in firstCandle),
      );

      if (missingFields.length > 0) {
        throw new Error(
          `Missing fields in chart data: ${missingFields.join(", ")}`,
        );
      }
      ...
      data.data.forEach((candle: any, index: number) => {
        ...
        if (candle.close < candle.low || candle.close > candle.high) {
          validationErrors.push(
            `Candle ${index}: close outside high-low range`,
          );
        }
      });
  ...
  if (error) {
    return (
      <div
        className="border rounded-sm p-4"
        style={{ backgroundColor: "#0a0e27", borderColor: "#ff1f1f" }}
      >
        <div
          className="font-mono text-xs uppercase tracking-widest mb-2"
          style={{ color: "#ff7f7f" }}
