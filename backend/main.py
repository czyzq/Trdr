...
# EXTRACTED TO api/routes/settings.py - using router below
...
# @app.get("/api/strategies")  # EXTRACTED to api/routes/strategies.py
...
# @app.get("/api/strategy/{symbol}")  # EXTRACTED
...
# @app.post("/api/strategy/{symbol}")  # EXTRACTED
...
# EXTRACTED TO api/routes/strategies.py - using router below
...
# @app.post("/api/strategies/save")  # EXTRACTED
...
# @app.get("/api/strategies/load/{strategy_id}")  # EXTRACTED
...
# @app.post("/api/strategies/backtest-json")  # EXTRACTED
    ...
    try:
        ...
        if take_profit is not None and stop_loss is not None:
            ...
            if signal:
                ...
                try:
                    candles = await get_cached_candles(symbol, "60", 50, data_provider)
...
async def close_trade(position_id: str):
    ...
    try:
        ...
        candles = await get_cached_candles(symbol, "60", 5, data_provider)  # 5 candles = ~5 hours
...
# MOVED TO backend/api/routes/news.py for better organization and to avoid rate limit issues during development/testing
...
# EXTRACTED TO api/routes/market.py - using router below
...
# EXTRACTED TO api/routes/market.py - with aggregation logic
